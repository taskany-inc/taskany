...
const jsToken = (token) => `export const ${camelCase(token)} = 'var(--${paramCase(token)})';`;
...
const jsTokensFile = (theme, file) => {
    ...
    for (const token in theme) {
        fileContent.push(jsToken(token));
        fileContent.push('\n');
    }
...
const cssTokensFile = (themeName, theme, file, tabs = 4) => {
    const fileContent = [
...
(async (t) => {
    ...
    for (const theme in t) {
        writers.push(cssTokensFile(theme, t[theme], join(themesFolder, theme, 'index.ts')));
    }
